...
**Frases de Ancoragem:**
""" + chr(10).join([f"- {frase}" for frase in driver.ancoragem_personalizada]) + f"""
...
""" + chr(10).join([f"- {driver['driver_base']['nome']}" for driver in sequencia['sequencia_drivers']['fase_1_despertar']]) + f"""

### Fase 2: Desejo (Amplificação)
""" + chr(10).join([f"- {driver['driver_base']['nome']}" for driver in sequencia['sequencia_drivers']['fase_2_desejo']]) + f"""

### Fase 3: Decisão (Pressão)
""" + chr(10).join([f"- {driver['driver_base']['nome']}" for driver in sequencia['sequencia_drivers']['fase_3_decisao']]) + f"""

### Fase 4: Direção (Caminho)
""" + chr(10).join([f"- {driver['driver_base']['nome']}" for driver in sequencia['sequencia_drivers']['fase_4_direcao']]) + f"""

## Cronograma de Implementação

""" + chr(10).join([f"**{fase.replace('_', ' ').title()}**: {', '.join(drivers_fase)}" for fase, drivers_fase in sequencia['cronograma_instalacao'].items()]) + f"""

## Métricas de Acompanhamento

### Engajamento
""" + chr(10).join([f"- {metrica}" for metrica in sequencia['metricas_acompanhamento']['metricas_engajamento']]) + f"""

### Conversão
""" + chr(10).join([f"- {metrica}" for metrica in sequencia['metricas_acompanhamento']['metricas_conversao']]) + f"""

### Psicológicas
""" + chr(10).join([f"- {metrica}" for metrica in sequencia['metricas_acompanhamento']['metricas_psicologicas']]) + f"""
